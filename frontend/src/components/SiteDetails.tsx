...
const PlugsList: React.FC = () => {
  ...
  const [siteSettings, setSiteSettings] = useState<SiteSetting | undefined>(
    undefined,
  )
  ...
  useEffect(() => {
    ...
    // Directly reference the site document with the given id
    const siteDocRef = doc(firestore, "sitesettings", id)

    // Listen for real-time updates on the site document
    const unsubscribe = onSnapshot(siteDocRef, (docSnapshot) => {
      if (docSnapshot.exists()) {
        const siteData = SiteSetting.fromJSON(docSnapshot.data())
